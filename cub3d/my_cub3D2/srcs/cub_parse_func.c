...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/15 13:22:20 by leng-chu          #+#    #+#             */
/*   Updated: 2024/01/15 13:35:02 by leng-chu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			parse_resolution(char *line, int *x, int *y)
{
	...
	if (*x != -1 && *y != -1)
	{
		return (return_message_i("You cannot have multiples resolution.",
		NULL, 1));
	}
	*x = ft_atoi_parse(line, &i);
	*y = ft_atoi_parse(line, &i);
	if (line[i] != '\0' || *x < 0 || *y < 0)
		return (return_message_i("Invalid resolution.", NULL, 1));
	return (0);
}
...
int			parse_textures(char *line, char **textures, const char *dir)
{
	...
	if (*textures)
	{
		return (return_message_i((char*)dir,
	...
	if (line[x] == '.' && line[x + 1] == '/')
	{
		...
		return (return_message_i("No path find for ", (char*)dir, 1));
	return (0);
}
...
int			parse_color_rgb(char *line, t_colorxpm *color, const char *details)
{
	...
	if (line[x] != ',')
		return (return_message_i((char*)details, " invalid.", 1));
	x++;
	...
	{
		return (return_message_i("Wrong color parameters for ",
		(char*)details, 1));
	}
...
int			parse_xpmcolor(char *line, t_colorxpm *color, const char *details)
{
	...
	{
		return (return_message_i((char*)details,
	...
	if (ft_isdigit(line[x]))
		...
		return (return_message_i((char*)details, " info not found.", 1));
	return (0);
}
